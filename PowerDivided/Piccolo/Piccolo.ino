...
void loop() {
  ...
  for(x=0; x<8; x++) 
  {
    ...
    if(level < 0L)      c = 0;
    else if(level > 9) c = 9; // Allow dot to go a couple pixels off top
...
//      //String message = "A" + String(fireTriggerDuration);
//      Wire.write(message.c_str());              // sends one byte
